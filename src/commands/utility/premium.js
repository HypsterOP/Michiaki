...
module.exports = class PremiumCommand extends Command {
  ...
  async do(message, args) {
    ...
    premKey.findOne({ Key: key }, async (err, data) => {
      if (!data)
        return message.channel.send({ content: "That key does not exist" });
      prem.findOne({ Guild: message.guild.id }, async (err, data) => {
        if (!data) {
          ...
          this.client.channels.cache
            .get("878567993901191179")
            .send({ content: `${message.guild.name} got premium.` })(
              await message.guild.fetchOwner()
            )
            .send({
              content: `Your server ${message.guild.name} is now premium.`,
            });
        } else
